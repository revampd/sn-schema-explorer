...
import { svg, root, zoom } from './canvas.js';
...
export function updateIndicators() {
  ...
  function spreadPills(pills, isHorizontal) {
    ...
    const MAX = isHorizontal ? cvW - PILL_W - MM_W : cvH - PILL_H - MM_H;
    const MIN = isHorizontal ? EL_W : MARGIN;
    pills.forEach(p => {
  ...
  candidates.forEach(c => {
    ...
    pill.addEventListener('click', ev => {
      ...
      svg
        ...
        .call(
          zoom.transform,
          d3.zoomIdentity.translate(cvW / 2 - tp.x * scale, cvH / 2 - tp.y * scale).scale(scale)
        )
