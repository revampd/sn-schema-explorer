...
import { INSP_OPEN_BY_DEFAULT } from './constants.js';
import { render } from './render.js';
import { computeNeighbourhood } from './compute.js';
import { typeBadgeColor, typeLabel } from './render.js';
import { openPanel, closePanel } from './canvas.js';
...
export function clearSelection() {
  ...
  const totalAvailable =
    uiState.filterConditions.length === 0
...
export function fillInspector(d) {
  ...
  const _filtOut = type =>
    _adj
      ...
      : graphState.graphData.edges.filter(
          e => (e.source?.id ?? e.source) === id && e.type === type
        );
  const _filtIn = type =>
    _adj
      ? adj.in.filter(e => e.type === type)
      : graphState.graphData.edges.filter(
          e => (e.target?.id ?? e.target) === id && e.type === type
        );
  const _findOut = type =>
    _adj
  ...
  const inheritanceChain = [];
  {
    ...
    for (let depth = 0; depth < 20; depth++) {
      ...
      const e = curAdj
        ...
        : graphState.graphData.edges.find(
            ed => (ed.source?.id ?? ed.source) === cur && ed.type === 'extends'
          );
      if (!e) break;
      const parentId = e.target?.id ?? e.target;
      if (seen.has(parentId)) break;
      seen.add(parentId);
      const parentNode = _nb
        ? _nb.get(parentId)
        : graphState.graphData.nodes.find(n => n.id === parentId);
      if (!parentNode) break;
  ...
  function makeEdgeItem(tableId, sub, cls) {
    ...
    const target = el('div', 'el-target');
    setText(target, tableId);
    const field = el('div', 'el-field');
    setText(field, sub || '');
  ...
  function makeSection(sectionKey, title, items) {
    ...
    setText(titleSpan, title);
    const toggle = el('span', 'insp-toggle');
    toggle.textContent = '+';
  ...
  const nodeLabel = nid =>
    (_nb ? _nb.get(nid) : graphState.graphData.nodes.find(n => n.id === nid))?.label ?? nid;

  const byTargetLabel = (a, b) => {
    const tl = nodeLabel(a.target?.id ?? a.target).localeCompare(
      nodeLabel(b.target?.id ?? b.target)
    );
    return tl !== 0 ? tl : (a.label || a.field || '').localeCompare(b.label || b.field || '');
  };
  const bySourceLabel = (a, b) => {
    const sl = nodeLabel(a.source?.id ?? a.source).localeCompare(
      nodeLabel(b.source?.id ?? b.source)
    );
  ...
  if (d.fields?.length) {
    ...
    sortedFields.forEach(f => {
      ...
      const row = el(
        'div',
        'insp-field-row' +
          (src ? ' insp-field-inherited' : '') +
          (isCustomField ? ' insp-field-row--custom' : '')
      );
      row.dataset.field = f.name;
      const names = el('div', 'insp-field-names');
      const fname = el('span', 'insp-field-name');
      setText(fname, f.name);
      if (isCustomField) {
        const badge = el('span', 'insp-custom-badge');
        badge.textContent = 'custom';
        fname.appendChild(badge);
      }
      const flabel = el('span', 'insp-field-label');
      setText(flabel, f.label);
      names.appendChild(fname);
      names.appendChild(flabel);
      ...
      const ftype = el('span', 'insp-field-type');
      setText(ftype, typeLabel(f.type));
      ftype.style.color = col;
      ftype.title = f.type;
    ...
    const customCount = Settings.isEnabled('customHighlight')
      ? sortedFields.filter(f => Settings.isCustomName(f.name)).length
      : 0;
    const customSuffix = customCount > 0 ? `, ${customCount} custom` : '';
    const titleStr =
      isFlattened && inheritedCount > 0
  ...
  if (inheritedFields.length) {
    ...
    for (const [ancId, group] of byAncestor) {
      ...
      setText(
        subId,
        `${ancId} · ${group.fields.length} field${group.fields.length === 1 ? '' : 's'}`
      );
    ...
    const sec = makeSection('inherited-fields', `Inherited fields (${inheritedFields.length})`, [
      grid,
    ]);
    if (sec) ic.appendChild(sec);
  }
  ...
  refsInSection = edgeSec(
    'refs-in',
    'Referenced by',
    [...inR].sort(bySourceLabel),
    src2,
    e => 'via ' + (e.field || e.label || ''),
    ''
  );
  ...
  if (Settings.isEnabled('inheritedRefsInspector') && inheritanceChain.length) {
    ...
    inheritanceChain.forEach((anc, ancIdx) => {
      ...
      const ancAdj = _adj
        ? _adj.get(ancId) || { out: [], in: [] }
        : { out: graphState.graphData.edges, in: graphState.graphData.edges };
      const ancOut = (
        _adj
          ? ancAdj.out.filter(e => e.type === 'reference')
          : graphState.graphData.edges.filter(
              e => (e.source?.id ?? e.source) === ancId && e.type === 'reference'
            )
      )
        .filter(e => !ownRefOutTargets.has(e.target?.id ?? e.target))
        .sort(byTargetLabel);
      ...
      const ancIn = (
        _adj
          ...
          : graphState.graphData.edges.filter(
              e => (e.target?.id ?? e.target) === ancId && e.type === 'reference'
            )
      )
        .filter(e => !ownRefInSources.has(e.source?.id ?? e.source))
        .sort(bySourceLabel);

      if (ancOut.length) {
        const items = ancOut.map(e => {
          const item = makeEdgeItem(
            tgt(e),
            (e.field || e.label || '') + ' (from ' + ancLabel + ')',
            'inherited'
          );
        ...
        const sec = makeSection(
          'inh-refs-out-' + ancIdx,
          'References out — inherited from ' + ancLabel + ' (' + ancOut.length + ')',
          items
        );
        if (sec) {
      ...
      if (ancIn.length) {
        const items = ancIn.map(e => {
          const item = makeEdgeItem(
            src2(e),
            'via ' + (e.field || e.label || '') + ' (targets ' + ancLabel + ')',
            'inherited'
          );
          item.dataset.inheritedFrom = ancId;
          return item;
        ...
        const sec = makeSection(
          'inh-refs-in-' + ancIdx,
          'Referenced by (targets ' + ancLabel + ') — ' + ancIn.length,
          items
        );
        if (sec) {
          sec.classList.add('insp-section-inherited');
  ...
  if (ciRelOut.length || ciRelIn.length) {
    ...
    const items = all.map(r =>
      makeEdgeItem(r.otherId, r.verbCentre + (r.verbOther ? '  ↔  ' + r.verbOther : ''), 'cmdbrel')
    );
    const sec = makeSection(
      'cirel',
  ...
  edgeSec('views', 'DB views', [...viewOf].sort(byTargetLabel), tgt, () => 'view member', 'view');
}
