import { graphState, uiState, serializeState, restoreState, setRestoreCallback } from '../../core/state.js';
...
export const SavedViews = (() => {
  ...
  function saveAll(obj) {
    ...
    catch (e) { console.warn('SavedViews: localStorage write failed', e); }
  }
  ...
  function fingerprint(graph) {
    ...
    const sample = graph.nodes.slice(0, 50).map(n => n.id).sort().join('|');
    let h = 0;
  ...
  function setFingerprint(fp) { currentFp = fp; notify(); }
...
function buildViewsList() {
  ...
  if (!views.length) {
    list.replaceChildren(
      h('div', { class: 'views-empty' }, 'No saved views yet.')
    );
    return;
  }
  ...
  for (const v of views) {
    ...
    frag.appendChild(
      h('div', { class: 'view-item', dataId: v.id, title: v.name },
        h('span', { class: 'view-name' }, v.name),
        h('span', { class: 'view-meta' }, dateLabel),
        h('button', { class: 'view-delete', dataAction: 'delete',
          dataId: v.id, title: 'Delete this view' }, '×')
      )
    );
  }
