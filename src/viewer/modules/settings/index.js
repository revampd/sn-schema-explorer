...
export const Settings = (() => {
  ...
  function notify(key, value) {
    ...
    anySubscribers.forEach(fn => { try { fn(key, value); } catch {} });
  }
  ...
  function getFontScale() {
    try { return parseInt(localStorage.getItem(FONT_SCALE_KEY), 10) || 100; } catch { return 100; }
  }
  ...
  function initMaxPngScale(detectedArea) {
    if (detectedArea >= 67108864) _maxPngScaleDefault = 200; // 64 MP+ — capable desktop browser
    else if (detectedArea >= 16777216) _maxPngScaleDefault = 50;  // 16 MP — current default
    else _maxPngScaleDefault = 20;                                 // < 16 MP — constrained fallback
  }
  function getMaxPngScale() {
    try { return parseInt(localStorage.getItem(MAX_PNG_SCALE_KEY), 10) || _maxPngScaleDefault; } catch { return _maxPngScaleDefault; }
  }
...
Settings.registerFeature({
  ...
  description: 'Render reference field names alongside edges in Schema Map for the selected table\'s neighbourhood. Useful for tracing dot-walks visually; off by default to keep dense graphs readable.',
  default:     false,
  category:    'display'
});
...
Settings.registerFeature({
  ...
  description: 'When hovering a table, dim everything that is not directly connected to it. Off by default to keep the canvas calm.',
  default:     false,
  category:    'display'
});
...
Settings.registerFeature({
  ...
  description: 'When you click a table, smoothly recenter and zoom the canvas to fit its neighbourhood. Off by default — most users prefer keeping their current viewport.',
  default:     false,
  category:    'behaviour'
});
...
Settings.registerFeature({
  ...
  description: 'Empties the search box when you click a table from the sidebar or canvas, so the next search starts fresh.',
  default:     true,
  category:    'behaviour'
});
...
export function renderSettingsModal() {
  ...
  const CATEGORY_META = {
    ...
    behaviour:   { title: 'Behaviour',
                   desc: 'How the application responds to your actions.' },
    ...
    export:      { title: 'Export',
                   desc: 'Settings that control export quality and limits.' },
  };
  ...
  for (const cat of CATEGORY_ORDER) {
    ...
    if (cat === 'export') {
      ...
      const minBtn = h('button', { class: 'setting-stepper-btn', title: 'Decrease max PNG scale',
        onClick: () => {
      ...
      minBtn.disabled = cur <= 10;
      const plusBtn = h('button', { class: 'setting-stepper-btn', title: 'Increase max PNG scale',
        onClick: () => {
          cur = Settings.setMaxPngScale(cur + 10);
          valEl.textContent = cur + '\xD7';
          minBtn.disabled = cur <= 10;
      ...
      section.appendChild(
        h('div', { class: 'setting-row' },
          h('div', { class: 'setting-row-text' },
            ...
            h('div', { class: 'setting-row-desc' }, 'Upper limit for the PNG resolution slider in the Export menu. Default 50\xD7. Range: 10–200\xD7, step 10\xD7. The browser’s 16 MP pixel cap still applies regardless.')
          ),
    ...
    if (cat === 'display') {
      ...
      const minBtn = h('button', { class: 'setting-stepper-btn', title: 'Decrease font size',
        onClick: () => {
      ...
      minBtn.disabled = cur <= 80;
      const plusBtn = h('button', { class: 'setting-stepper-btn', title: 'Increase font size',
        onClick: () => {
          cur = Settings.setFontScale(cur + 10);
          valEl.textContent = cur + '%';
          minBtn.disabled = cur <= 80;
      ...
      section.appendChild(
        h('div', { class: 'setting-row' },
          h('div', { class: 'setting-row-text' },
            ...
            h('div', { class: 'setting-row-desc' }, 'Scale text in the inspector, sidebar, and modals. Range: 80–150%, step 10%.')
          ),
          h('div', { class: 'setting-stepper' }, minBtn, valEl, plusBtn)
        )
      );
  ...
  body.appendChild(
    h('div', { class: 'settings-actions' },
      h('button', {
        ...
        onClick: () => {
          if (!confirm('Reset all settings to defaults? This will not delete your saved views or data.')) return;
          try {
