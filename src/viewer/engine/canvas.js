...
export const zoom = d3.zoom().scaleExtent([.04, 5]).on('zoom', e => {
...
export function setMinimapUpdater(fn) { _minimapUpdater = fn; }

export function openPanel(id)  { document.getElementById(id).classList.add('open');    showScrim(); }
export function closePanel(id) { document.getElementById(id).classList.remove('open'); hideScrim(); }

export function showScrim() { Dom.scrim.classList.add('visible'); }
export function hideScrim() {
  const anyOpen = Dom.sidebar.classList.contains('open') ||
                  Dom.inspector.classList.contains('open');
...
Dom.scrim.addEventListener('click', () => {
  closePanel('sidebar'); closePanel('inspector');
