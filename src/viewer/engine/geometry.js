export const Geometry = (() => {
  ...
  function buildPath(sx, sy, tx, ty, shh, thh, offset) {
    ...
    if (centreDist < overlapDist) {
      return { sp:{x:sx,y:sy}, tp:{x:tx,y:ty}, cx:null, cy:null, straight:true, degenerate:true };
    }
    const sp2 = clip(sx,sy,tx,ty,shh), tp2 = clip(tx,ty,sx,sy,thh);
    const epDx = tp2.x-sp2.x, epDy = tp2.y-sp2.y;
