...
const TL_ROW_HEIGHT_HINT = (typeof matchMedia === 'function'
  && matchMedia('(pointer: coarse)').matches) ? 58 : 50;
const TL_OVERSCAN   = 6;
let _tableData       = [];
let _tableDataAll    = [];
...
function _rowH() { return _hintMode ? TL_ROW_HEIGHT_HINT : TL_ROW_HEIGHT; }
...
export function tlRenderVisible() {
  ...
  const firstIdx = Math.max(0,
    Math.floor(scrollTop / _rowH()) - TL_OVERSCAN);
  const lastIdx  = Math.min(_tableData.length,
    Math.ceil((scrollTop + clientH) / _rowH()) + TL_OVERSCAN);

  const frag = document.createDocumentFragment();
  for (let i = firstIdx; i < lastIdx; i++) {
    const n = _tableData[i]; if (!n) continue;
...
export function getTableDataAll() { return _tableDataAll; }
export function setTableData(arr) { _tableData = arr; }
