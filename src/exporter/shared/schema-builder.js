  ...
  function () {
    ...
    function build(input, options) {
      ...
      for (var iw = 0; iw < sysDbViewTable.length; iw++) {
        ...
        var viewName =
          vt.view && vt.view.displayValue
            ? vt.view.displayValue
            : vt.view && vt.view.name
              ? vt.view.name
              : null;
      ...
      for (var ir = 0; ir < sysRelation.length; ir++) {
        ...
        var toTable =
          parseRelationshipTargetTable(rr.query_from) ||
      ...
      var deepestChain = 0;
      var refSourceTables = {};
      for (var ie = 0; ie < referenceEdges.length; ie++)
        refSourceTables[referenceEdges[ie].source] = true;
      ...
      for (var iz = 0; iz < nodes.length; iz++) {
        ...
        if (n._isView) {
          viewNodeCount++;
          continue;
        } // skip view-specific stats below
      ...
      var coverage = {
        ...
        tables_with_references_pct: pureTableCount
          ? +((100 * tablesWithRefs) / pureTableCount).toFixed(1)
          : 0,
      ...
      ciRelationships.sort(function (a, b) {
        var pa = String(a.parentClass),
          pb = String(b.parentClass);
        if (pa !== pb) return pa < pb ? -1 : 1;
        var ca = String(a.childClass),
          cb = String(b.childClass);
