...
const config = {
    ...
    output:     args.output     || process.env.SN_OUTPUT    || 'sn_schema_export.json',
...
async function fetchAllViaTableApi() {
    ...
    const [sysDbObjectRaw, sysDbViewRaw, sysGlideObjectRaw] = await Promise.all([
        fetchTableAll('sys_db_object',    null,
            ['sys_id','name','label','super_class','sys_scope','is_extendable','access','scriptable_table'],
            prog('sys_db_object')),
...
(async function main() {
    ...
    try {
        // Stream output to file to avoid materialising the full JSON as one
        // Node string (V8 has a ~512 MB string cap; streaming keeps memory
        // usage predictable as schemas grow).
        const input = await fetchAllViaTableApi();
        log('Building schema locally and streaming to ' + config.output + '…');

        if (config.pretty) {
            // Pretty mode requires the full tree for indentation; use classic build.
            const schema = SchemaBuilder.build(input);
            fs.writeFileSync(config.output, JSON.stringify(schema, null, 2), 'utf8');
