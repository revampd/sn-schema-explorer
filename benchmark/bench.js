...
copyFileSync(join(dist, 'sn_schema_explorer.html'), join(dist, 'bench-d3-790.html'));
...
execSync('npm install --save-dev d3@7.8.5 --no-audit --fund=false', {
  cwd: root,
  stdio: 'inherit',
});
execSync('node build.js full', { cwd: root, stdio: 'inherit' });
copyFileSync(join(dist, 'sn_schema_explorer.html'), join(dist, 'bench-d3-785.html'));

// Restore 7.9.0
console.log('\n── Restoring D3 7.9.0 ─────────────────────────────────────────');
execSync('npm install --save-dev d3@7.9.0 --no-audit --fund=false', {
  cwd: root,
  stdio: 'inherit',
});
  ...
  for (let run = 0; run < RUNS; run++) {
    ...
    for (let i = 0; i < 300; i++) {
      ...
      if (nodes === lastNodes && edges === lastEdges) {
        stableCount++;
        if (stableCount >= 5) {
          stableTime = Date.now() - stableStart;
          break;
        }
      } else {
        stableCount = 0;
        ...
        return new Promise(resolve => {
          ...
          if (el) {
            ...
            for (let dx = 0; dx < 200; dx += 10) {
              el.dispatchEvent(
                new MouseEvent('mousemove', { clientX: cx + dx, clientY: cy, bubbles: true })
              );
            }
            el.dispatchEvent(
              new MouseEvent('mouseup', { clientX: cx + 200, clientY: cy, bubbles: true })
            );
          }
        });
      },
    ...
    console.log(
      `    Run ${run + 1}: first-node=${timeToFirstNode}ms  stable=${stableTime}ms  p50=${p50.toFixed(1)}ms  p95=${p95.toFixed(1)}ms  fps≈${avgFps}  errors=${errors.length}`
    );
